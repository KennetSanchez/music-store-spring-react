    ...
    return (
        <div className={`w-full h-[32rem] rounded-md drop-shadow-md bg-neutral-900 flex flex-col justify-between items-center ${hoverStyle}`}>
            ...
            <div className={"w-full h-1/3 p-2"}>
                <h3 className={"tracking-wider"}>{props.name}</h3>
                ...
                <div className={"flex justify-between items-center mt-2"}>
                    ...
                    <div className={"flex flex-col justify-between items-end"}>
                        <h3 className={"tracking-wider"}>${props.price}</h3>
                        <p className={"font-light text-sm"}>{props.shippingIncluded ? "shipping included" : "+shipping fee"}</p>
                    </div>
                </div>
            </div>
        </div>
